...
class KrakenService(
    ...
    fun retrieveTrades() {
        ...
        exchange.exchangeSymbols.intersect(tradeConfiguration.symbols).forEach { pair ->
            try {
                ...
                logRepository.save(Log(exchange = exchangeName, message = "${e.message}", isError = true, timestamp = normalizeTime(Instant.now().toEpochMilli()), pair = pair.toString(), type = Type.TRADE))
            }
        }
    }
    ...
    private fun generateOHLC(trades: List<Trade>, interval: Int, pair: String, exchange: String) {
        GlobalScope.launch { 
            logger.info { "Loading: ${trades.size}, $interval, $pair, $exchange" }
            ohlcService.loadFromTrades(trades, interval, pair, exchange)
            logger.info { "Loaded: ${trades.size}, $interval, $pair, $exchange" }
        }
    }

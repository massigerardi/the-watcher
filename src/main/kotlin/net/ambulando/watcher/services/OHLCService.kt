    ...
    fun loadFromTrades(trades: List<Trade>, interval: Int, pair: String, exchange: String): OHLCs {
        ...
        return candles.let { 
            ...
            return OHLCs(interval = interval, ohlcs = it, id = UUID.randomUUID().toString(), pair = pair, exchange = exchange).apply {
                ...
                logRepository.save(Log(id = UUID.randomUUID().toString(), timestamp = normalizeTime(it.sortedBy { it.time }.last().time), message = "Loaded $pair for $interval from $exchange", pair = pair, exchange = exchange, type = Type.OHLC))
            }
        }
    }
    ...
    fun loadFromCSV(interval: Int, file: File, pair: String, exchange: String): OHLCs =
        file.bufferedReader(Charset.defaultCharset()).readLines()
            ...
            .let {
                return OHLCs(interval = interval, ohlcs = it, id = UUID.randomUUID().toString(), pair = pair, exchange = exchange).apply {
                    ohlcsRepository.save(this)
                    logRepository.save(Log(id = UUID.randomUUID().toString(), timestamp = it.sortedBy { it.time }.last().time, message = "Loaded $pair for $interval from $exchange", pair = pair, exchange = exchange, type = Type.OHLC))
                }
            }
